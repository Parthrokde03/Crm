import { Table, Group, Text, Pagination, Select, Checkbox, ActionIcon, TextInput, Loader, Stack, Paper, Box } from '@mantine/core';
...
export function DataTable<T>({
  ...
  sortBy, sortOrder, isLoading, selectable, selectedIds, onSelect,
  onPageChange, onPageSizeChange, onSort, onSearch, searchValue,
  ...
  const handleSelectAll = () => {
    if (!onSelect) return;
    onSelect(allSelected ? new Set() : new Set(data.map(getId)));
  };
    ...
    <Paper withBorder radius="md" style={{ overflow: 'hidden' }}>
      ...
      <Table.ScrollContainer minWidth={800}>
        <Table highlightOnHover verticalSpacing="sm" horizontalSpacing="md">
          <Table.Thead>
            <Table.Tr style={{ background: 'var(--mantine-color-gray-light)' }}>
              {selectable && (
                <Table.Th w={40}>
              ...
              {actions && <Table.Th w={80} style={{ textAlign: 'right' }}>Actions</Table.Th>}
            </Table.Tr>
          </Table.Thead>
          <Table.Tbody>
            {isLoading ? (
              ...
              data.map(row => {
                ...
                return (
                  <Table.Tr key={id}>
                    {selectable && (
                      <Table.Td>
                        <Checkbox checked={selectedIds?.has(id)} onChange={() => handleSelectRow(id)} aria-label={`Select row ${id}`} />
                      </Table.Td>
                    )}
                    {columns.map(col => (
                    ...
                    {actions && <Table.Td style={{ textAlign: 'right' }}>{actions(row)}</Table.Td>}
                  </Table.Tr>
                );
              })
            )}
