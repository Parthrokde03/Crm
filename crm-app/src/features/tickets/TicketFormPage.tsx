...
export function TicketFormPage() {
  ...
  const form = useForm({
    ...
    validate: {
      subject: (v) => (v.trim() ? null : 'Required'),
      description: (v) => (v.trim() ? null : 'Required'),
      contactId: (v) => (v ? null : 'Contact is required'),
    },
  });
  ...
  const handleSubmit = form.onSubmit((values) => {
    ...
    if (isEdit) updateMutation.mutate({ id: id!, data: payload }, { onSuccess: () => navigate('/tickets') });
    else createMutation.mutate(payload, { onSuccess: () => navigate('/tickets') });
  });

  return (
    <>
      <PageHeader title={isEdit ? 'Edit Ticket' : 'New Ticket'} subtitle={isEdit ? `Editing ${existing?.subject ?? ''}` : 'Create a new support ticket'} breadcrumbs={[{ label: 'Dashboard', to: '/' }, { label: 'Tickets', to: '/tickets' }, { label: isEdit ? 'Edit' : 'New' }]} />
