import { Button, ActionIcon, Menu, Group, Text, Box, Badge } from '@mantine/core';
...
const columns: Column<User>[] = [
  {
    ...
    render: (r) => (
      <Group gap="sm">
        <EntityAvatar name={`${r.firstName} ${r.lastName}`} src={r.avatar} size="sm" color="indigo" />
        <Box>
  ...
  {
    key: 'role', label: 'Role',
    render: (r) => <Badge variant="light" size="sm" radius="sm">{ROLE_LABELS[r.role.name] ?? r.role.name}</Badge>,
  },
  {
    key: 'isActive', label: 'Status',
    render: (r) => <StatusBadge status={r.isActive ? 'active' : 'inactive'} colorMap={{ active: 'green', inactive: 'gray' }} />,
  },
  {
    key: 'lastLoginAt', label: 'Last Login',
    render: (r) => <Text size="sm" c={r.lastLoginAt ? undefined : 'dimmed'}>{r.lastLoginAt ? new Date(r.lastLoginAt).toLocaleDateString() : 'Never'}</Text>,
  },
];
...
export function UsersListPage() {
  ...
  return (
    <>
      <PageHeader
        title="Users"
        subtitle={`${listQuery.data?.meta.total ?? 0} team members`}
