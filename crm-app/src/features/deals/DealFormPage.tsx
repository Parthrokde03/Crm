import { Paper, TextInput, Select, NumberInput, Textarea, Button, Group, Stack, SimpleGrid, Center, Loader, Divider, Text } from '@mantine/core';
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={isEdit ? `Editing ${existing?.title ?? ''}` : 'Create a new deal'}
        breadcrumbs={[{ label: 'Dashboard', to: '/' }, { label: 'Deals', to: '/deals' }, { label: isEdit ? 'Edit' : 'New' }]}
      />
      <Paper withBorder p="xl" radius="md" maw={800}>
        <form onSubmit={handleSubmit}>
          <Stack gap="lg">
            <Text size="xs" c="dimmed" tt="uppercase" fw={600} style={{ letterSpacing: '0.05em' }}>Deal Information</Text>
            <TextInput label="Deal Title" placeholder="Enterprise License Agreement" required {...form.getInputProps('title')} />
            ...
            <Divider />
            <Text size="xs" c="dimmed" tt="uppercase" fw={600} style={{ letterSpacing: '0.05em' }}>Associations</Text>
            ...
            <Divider />
            <Textarea label="Description" placeholder="Add deal details..." rows={3} {...form.getInputProps('description')} />
